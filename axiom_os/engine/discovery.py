...
if TYPE_CHECKING:
    from axiom_os.engine.forms import FormCandidate
...
class DiscoveryEngine:
    ...
    def distill(
        ...
        # P0: 扩展 unary_operators 以支持超越函数 (exp, log, sin, cos)
        if self.use_pysr and HAS_PYSR:
            try:
                model = PySRRegressor(
                    niterations=niterations,
                    binary_operators=["+", "*", "-", "/"],
                    unary_operators=["square", "sqrt", "abs", "exp", "log", "log1p", "sin", "cos"],
                )
    ...
    def discover_gflownet(
        ...
        """
        from axiom_os.engine.gflownet import GFlowNetDiscovery
    ...
    def discover_multivariate(
        ...
        allowed_input_indices: Optional[Any] = None,
        ...
        """
        from axiom_os.engine.forms import MultivariatePolyForm, aic, bic
        ...
        # P0: 当 use_pysr 时，先尝试 PySR（支持 exp/log/sin/cos）
        if self.use_pysr and HAS_PYSR and n >= 50:
            try:
                pairs = [(X[i], y[i]) for i in range(n)]
                formula = self.distill(
                    rcln_layer=None,
                    data_buffer=pairs,
                    input_units=[[0, 0, 0, 0, 0]] * d,
                    niterations=30,
                )
                if formula and len(str(formula).strip()) > 2:
                    safe = {"np": np, "sin": np.sin, "cos": np.cos, "sqrt": np.sqrt, "exp": np.exp, "log": np.log, "log1p": np.log1p, "abs": np.abs, "pow": np.power}
                    for i in range(min(d, 10)):
                        safe[f"x{i}"] = X[:, i]
                    pred_pysr = np.asarray(eval(formula, {"__builtins__": {}}, safe), dtype=np.float64).ravel()
                    if pred_pysr.shape[0] == n and np.all(np.isfinite(pred_pysr)):
                        mse = np.mean((pred_pysr - y) ** 2)
                        k = max(1, formula.count("+") + formula.count("*") + 2)
                        score = score_fn(n, mse, k)
                        if score < best_score:
                            best_formula = formula
                            best_pred = pred_pysr.copy()
                            best_coefs = {"form_type": "pysr"}
                            best_score = score
            except Exception:
                pass
        ...
        for cfg in configs:
            try:
                ...
                pred, coefs, formula = form.fit(
                    X, y, var_names=names, sample_weight=sample_weight,
    ...
    def discover_parametric(
        ...
        """
        from axiom_os.engine.forms import (
            FormCandidate,
            BATTERY_AGING_FORMS,
            aic,
            bic,
