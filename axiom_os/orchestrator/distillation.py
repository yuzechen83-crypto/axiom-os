"""
Policy Distillation - "Muscle Memory" Upgrade (from SPNN-Evo)
...
from .mpc import ImaginationMPC, angle_normalize, double_pendulum_H, step_env
...
class DoublePendulumEnv:
    ...
    def step(self, action: float) -> Tuple[np.ndarray, float, bool, dict]:
        """action: tau (scalar). Returns: obs, reward, done, info"""
        q_new, p_new = step_env(
            self._q, self._p,
            float(action),
            self.H,
            dt=self.dt,
...
class PolicyTrainer:
    ...
    def collect_mpc_data(
        ...
        """Run MPC on environment for N episodes. Returns number of transitions collected."""
    ...
    def train_student(
        ...
        """Train Student MLP to minimize MSE(Student(obs), MPC_Action). Returns epoch losses."""
    ...
    def run_dagger_iteration(
        ...
        """DAgger: Student drives, MPC labels. Returns (new_transitions, epoch_losses)."""
        steps_per_episode = steps_per_episode or self.env.max_steps
        new_count = 0
    ...
    def compute_policy_error(self, obs: np.ndarray, mpc_action: float) -> float:
