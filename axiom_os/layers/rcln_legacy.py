...
from typing import Optional, Callable, Union, NamedTuple, Any, List
...
# KAN: Symbolic evolution path (built-in, no external deps)
...
def _build_soft_shell(
    ...
    """
    Build soft shell with multiple evolution paths:
    
    Evolution Hierarchy (most to least specialized):
        KAN > FNO > CliffordTransformer > MERA > Spectral > Clifford > MLP
    
    Args:
        net_type: "kan", "fno", "clifford_transformer", "mera", "mlp"
        kan_grid_size: Grid size for KAN B-splines (larger = more expressive)
        kan_spline_order: Spline order for KAN (3 = cubic)
    """
    if net_type == "kan":
        return KANSoftShell(
            input_dim=input_dim,
...
class RCLNLayer(nn.Module):
    ...
    Supports multiple Soft Shell architectures for different evolution paths:
    
    Evolution Paths:
        KAN-RCLN (net_type='kan'): Symbolic, interpretable, discovery-ready
        FNO-RCLN (net_type='fno'): Resolution-invariant, global operators  
        Clifford-RCLN (use_clifford=True): Geometric, rotation-equivariant
        Spectral-RCLN (use_spectral=True): Frequency-domain, periodic physics
        MLP-RCLN (default): Classical, flexible baseline
    
    Architecture: y_total = y_hard + λ·y_soft
        y_hard: Physics equation (analytical, from domain knowledge)
        y_soft: Neural approximation (learned, from data)
    
    Features:
        - ActivityMonitor: Detects DiscoveryHotspots for formula crystallization
        - Hippocampus integration: Memory-guided perturbations
        - Uncertainty quantification: Know what we don't know
        - Formula extraction: KAN mode enables direct symbolic reading
    """
    ...
    def __init__(
        ...
        # Soft Shell: KAN > FNO > Spectral > Clifford > MLP
        # Evolution priority: Symbolic (KAN) > Operator (FNO) > Geometric (Clifford)
        if use_spectral is None:
            use_spectral = False
        if use_clifford is None:
            use_clifford = HAS_CLIFFORD and not use_spectral and net_type not in ("fno", "kan")
    ...
    def extract_formula(self, var_names: Optional[List[str]] = None) -> Optional[dict]:
        """
        Extract symbolic formula from the Soft Shell (KAN mode only).
        
        This is the key bridge between neural network and analytical physics.
        For KAN-RCLN, this reads the learned spline coefficients directly.
        For other modes, returns None (requires PySR or symbolic regression).
        
        Args:
            var_names: Optional list of variable names for formula readability
    ...
    def get_architecture_info(self) -> dict:
        ...
        Returns:
            dict with architecture details for logging/debugging
        """
        return {
            ...
            "kan_grid_size": self.kan_grid_size if self.net_type == "kan" else None,
            "kan_spline_order": self.kan_spline_order if self.net_type == "kan" else None,
        }
