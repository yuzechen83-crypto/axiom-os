"""
PDE Discovery Feature Library (from SPNN-Evo)
...
def compute_derivatives_2d(
    ...
    """Compute ux, uxx for u(t, x) on a 2D grid (t, x). Returns: ux, uxx"""
...
class VectorFeatureExtractor:
    ...
    Invariants: div_u, curl_u (ω), laplacian_omega, advection (u·∇)ω
    """
    ...
    def extract(self, u: np.ndarray, v: np.ndarray) -> dict:
        """u, v: (T, H, W) or (H, W). Returns dict with u_x, v_y, div_u, curl_u, laplacian_omega, advection, etc."""
...
def sparse_regression_pde(
    ...
    """Least-squares regression: target = Theta @ xi. Returns (xi, formula_str)"""
